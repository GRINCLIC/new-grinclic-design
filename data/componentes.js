...
<div class="mb-3">
  ...
  },
  {
    ...
    use:"Usarlo para claves y configuraciones sensibles. No incluye ver/ocultar porque por política interna la contraseña debe verse siempre como caracteres ocultos. Excepción documentada: la contraseña del login (entrada Pantalla pública) sí lo lleva, con la clase general <code>.gc-password-control</code> + <code>.gc-password-toggle</code> de <code>cplus/scss/_gc-forms.scss</code>; fuera del login no usarla sin decisión explícita. La regla la aplica data-gc-rule=\"password\": gc-validate.js la resuelve contra Validators.password (mínimo 8 caracteres y al menos un número, una minúscula y una mayúscula).",
...
<div class="column-filter-popover is-open" aria-hidden="false">
  ...
  ,
  {
    ...
    catalogExamples: [],
    implementations: [
      { module: "Login CPlus · genérica", agregar: null, file: "cplus/views/auth/_generico.php", detail: "Se entra directo, sin link: foto de ambiente con velo verde plano, logo GRINCLIC, usuario en texto plano y contraseña oculta con ojo dentro del campo, acción en el primario y legales con el isotipo al pie (diseño V3)" },
    ...
    use:"Usarla en pantallas públicas y centradas, sin chrome ni menú: login y, cuando se migren, recuperación de contraseña y el acceso auxiliar de operadores logísticos. La tarjeta interior es siempre <code>.form-card--auth</code>; la contraseña va dentro de <code>.gc-password-control</code> con su <code>.gc-password-toggle</code>; el usuario empresarial es un campo de texto plano; el enlace de recuperación va en <code>.gc-auth__recovery</code>, entre los campos y la acción, y la acción centrada en <code>.gc-auth__actions</code>.",
    ...
    <div class="form-card form-card--auth">
      ...
      <form action="validar.php" method="POST" autocomplete="off" novalidate onsubmit="return false;">
        <div class="mb-3">
          ...
          <input type="text" class="form-control" id="gc-demo-user-cli" name="user" required autocomplete="username" autocapitalize="off" spellcheck="false">
        </div>
    ...
    <div class="form-card form-card--auth">
      ...
      <form action="validar.php" method="POST" autocomplete="off" novalidate data-gc-login-form>
        <div class="mb-3">
          ...
          <input type="text" class="form-control" id="gc-login-user" name="user" required autocomplete="username" autocapitalize="off" spellcheck="false" data-gc-login-filtrar data-gc-login-usuario>
        </div>
