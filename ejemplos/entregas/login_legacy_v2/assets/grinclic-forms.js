(function(){
  ...
  function bindPasswordToggles(root){
    root.querySelectorAll("[data-gc-password-toggle]").forEach(function(button){
      if(button.dataset.gcPasswordBound) return;
      button.dataset.gcPasswordBound = "true";
      var target = document.querySelector(button.dataset.gcPasswordToggle);
      if(!target) return;
      var icon = button.querySelector("i");
      button.addEventListener("click", function(){
        var isHidden = target.type === "password";
        var label = button.getAttribute("data-gc-password-label") || "contraseña";
        target.type = isHidden ? "text" : "password";
        button.setAttribute("aria-pressed", isHidden ? "true" : "false");
        button.setAttribute("aria-label", (isHidden ? "Ocultar " : "Mostrar ") + label);
        if(icon){
